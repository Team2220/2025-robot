...
public class CommandXBoxWrapper {
  ...
  public Trigger joysticksTriggerLeft() {
    return new Trigger(
        () -> {
          return Math.abs(getLeftX()) > 0 || Math.abs(getLeftY()) > 0;
        });
  }
  ...
  public Trigger joysticksTriggerRight() {
    return new Trigger(
        () -> {
          return Math.abs(getRightX()) > 0 || Math.abs(getRightY()) > 0;
        });
  }
  ...
  public Command rumbleCommand(double value) {
    return Commands.startEnd(() -> setRumble(value), () -> setRumble(0));
  }
}
